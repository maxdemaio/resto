  ...
  return (
      <section id="menus" className="bg-[#895621] text-white px-3 md:px-[10rem] py-14">
        ...
        <div className="flex items-stretch justify-between w-full border-b mb-16">
          <button onClick={() => handleCategoryClick('Food')} className={`item grow p-4 font-mantra text-xl md:text-3xl basis-0 ${isButtonActive('Food')}`}>FOOD</button>
          <button onClick={() => handleCategoryClick('Drinks')} className={`item grow p-4 font-mantra text-xl md:text-3xl basis-0 ${isButtonActive('Drinks')}`}>DAY DRINKING</button>
          <button onClick={() => handleCategoryClick('More')} className={`item grow p-4 font-mantra text-xl md:text-3xl basis-0 ${isButtonActive('More')}`}>COFFEE & MORE</button>
        </div>
        <div>
